import {PanResponder, PanResponderGestureState, Dimensions, Animated} from 'react-native';
import {SwipeDirection, TabScreen} from '../screens/TabScreen';
import {animationConfig} from '../common/TransitionAnimationConfig';

const windowWidth = Dimensions.get('window').width;

function isHorizontalGesture(gestureState: PanResponderGestureState) {
  return Math.abs(gestureState.dx) > Math.abs(gestureState.dy * 2) && Math.abs(gestureState.vx) > Math.abs(gestureState.vy * 2);
}

function isMovingPrev(gestureState: PanResponderGestureState, tabScreen: TabScreen) {
  return (gestureState.dx > 0 || (gestureState.dx === 0 && gestureState.vx > 0.00000001)) && tabScreen.state.tabIndex > 0;
}

function isMovingNext(gestureState: PanResponderGestureState, tabScreen: TabScreen) {
...
function shouldAcceptSwipeGesture(gestureState: PanResponderGestureState) {
